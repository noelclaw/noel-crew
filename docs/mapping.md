# NoelCrew reaction, speech, and Claude hook mapping

This document explains how NoelCrew decides which pet reaction is shown, when the pet says something, and how Claude Code hooks and OpenCode plugin events map to reactions/messages.

## Important terms

- **Reaction**: one of the allowed NoelCrew reaction names, for example `thinking`, `success`, or `error`.
- **Speech/message**: a short text bubble sent with `noelcrew_say` or generated by a Claude hook.
...
NoelCrew currently accepts these reactions:
...
The universal Codex/NoelCrew spritesheet has these animation rows:
...
| --- | --- |
| Reaction only: `noelcrew_react({ reaction: "testing" })` | A stable randomized status line from the `testing` message pool, for example `Running the checks`. |
| Message only: `noelcrew_say({ message: "Done" })` | `Done` |
| Message + reaction: `noelcrew_say({ message: "Done", reaction: "success" })` | `Done` |
...
When NoelCrew Claude hooks are installed, these Claude Code hook events are registered:
...
For `PreToolUse` with `tool_name: "Bash"`, NoelCrew classifies the command as `testing` if the command contains one of these patterns:
...
If speech is throttled, NoelCrew still sends the reaction with `pet.react`. Example: if `Stop` happens twice within 20 seconds, the second one sends `success` as a reaction only instead of saying another success phrase.
...
When NoelCrew OpenCode setup is installed, OpenCode loads the `@noelclaw/opencode` plugin. Plugin hooks return immediately and schedule NoelCrew IPC calls in the background so OpenCode is not blocked by pet activity.
...
| Internal/self NoelCrew tools | NoelCrew MCP/status/say/react calls. | None. | No. |
...
| --- | --- | --- |
| `noelcrew_status` | none | Checks whether NoelCrew is running and which pet is targeted. Does not change pet state. |
| `noelcrew_react` | `{ reaction }` | Drives the reaction animation and may show a short randomized temporary bubble from that reaction's message pool. |
| `noelcrew_say` | `{ message, reaction? }` | Sends a message bubble, optionally with a reaction. The message is what appears in the bubble. |

### MCP message validation

`noelcrew_say` messages must be:
...
NoelCrew decides which pet gets the reaction/message as follows:
