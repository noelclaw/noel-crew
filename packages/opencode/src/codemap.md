...
- **opencode-plugin-runtime.ts**: Plugin hook implementations (229 lines). `createNoelCrewOpenCodeHooks()`, event classification, tool reaction mapping, lease management, throttling.
...
- **opencode-previews.ts**: Config entry builders (52 lines). `buildOpenCodeMcpEntry()`, `buildOpenCodePluginPreview()`, `buildOpenCodeInstructionPath()`, `validateNoelCrewPetArg()`.
