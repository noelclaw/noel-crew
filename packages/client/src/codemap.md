...
- **index.ts**: Main client implementation. `createNoelCrewClient()` factory, all client methods, result parsers (`parsePetListResult`, `parsePetInstallResult`), and `sendRequest()` for low-level IPC.
- **protocol.ts**: IPC protocol constants, request/response types, `parseIpcResponse()`, `validateReaction()`, `NoelCrewClientError` class.
