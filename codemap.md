# Repository Atlas: NoelCrew 2.0 Workspace

## Project Responsibility

NoelCrew is a pnpm/TypeScript monorepo for an Electron desktop companion app plus npm packages that let coding agents control animated desktop pets. The workspace provides a local IPC protocol, MCP server, CLI tooling, and editor-specific integrations for Claude Code and OpenCode.
...
| `packages/cli/` | User-facing NoelCrew CLI package. | [View Map](packages/cli/codemap.md) |
...
| `packages/mcp/` | MCP server package exposing NoelCrew tools to compatible agents. | [View Map](packages/mcp/codemap.md) |
...
| `packages/pi/src/` | Pi extension entry point, event classification, NoelCrew command parsing, and validation checks. | [View Map](packages/pi/src/codemap.md) |
| `packages/pet-format/` | Minimal package marker/type interface for NoelCrew pet package identity. | [View Map](packages/pet-format/codemap.md) |
...
2. Agent integrations (`packages/claude`, `packages/opencode`, `packages/pi`, and `packages/mcp`) emit pet commands through `@noelclaw/client`.
